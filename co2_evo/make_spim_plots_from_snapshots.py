...
def plot_erosion_slope_width_over_distance(snapshotdir, res, every=5):
    ...
    cb=colorbar(sMap, ax=axs[1],format=cb_formatter)
    cb.set_label('Years')
    ...
    savefig(snapshotdir+'/1-Erosion-slope-width-v-distance.png')
...
def plot_morphodynamics(snapshotdir, res):
    ...
    cNorm = Normalize(vmin=0,vmax=res['x'][0,-1])
    sMap = cm.ScalarMappable(norm=cNorm, cmap = cmap)

    fig, axs = subplots(3,1, figsize=(5,12),
                       sharex=True)
    ...
    axs[2].plot([res['years'][0], res['years'][-1]], [-res['dz_dt_base'],-res['dz_dt_base']], '--k')
    axs[2].tick_params(labelsize=10)

    tight_layout(h_pad=0, w_pad=0)
    savefig(snapshotdir+'/1-Morphodynamics.png')
...
def final_step_morphology(snapshotdir, res):
    ...
    figure()
    semilogy(res['width'][-1], res['slope'][-1], 'ko')
    w_i = res['width'][-1][0]
    s_i = res['slope'][-1][0]
    w_f = res['width'][-1][-1]
    ...
    plot(res['width'][-1], res['width'][-1]**m * 10**b, ':k')
    ...
    legend(['With CO2', 'Optimal: '+opt_str, 'Fit: '+fit_str])
    tight_layout()
    savefig(snapshotdir+'/1-Final-Morphology.png')
