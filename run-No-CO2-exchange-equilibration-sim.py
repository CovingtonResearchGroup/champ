...
for t in arange(ntimes):
    ...
    if t % 100 == 0:
        ...
        ylabel('Elevation (m)')
        legend(['h','z'])
        savefig('./without-CO2-figs/Elevation-Profile-'+timestep_str+'.png')
        ...
        plot(x,sim.Ca)
        legend(['w','a','Ca'])
        savefig('./without-CO2-figs/Concentration-Profile-'+timestep_str+'.png')
        ...
        tight_layout()
        savefig('./without-CO2-figs/Slope-'+timestep_str+'.png')
        #Create 3d XC fig
        fig = plt.figure()
