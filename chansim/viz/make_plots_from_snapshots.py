...
def get_results(plotdir):
    ...
    for snapnum, snapshot in enumerate(snapshots):
        ...
        for i, xc in enumerate(snap_sim.xcs):
            ...
            if snap_sim.flow_type[i] != "full":
                ...
                these_widths[i] = 0.0
        # Get data from nodes
        these_Ca_Ca_eq = np.zeros(len(snap_sim.x_arr))
    ...
    A = np.array(A)
    Pw = np.array(Pw)
    CO2_w = np.array(CO2_w)
    Ca = np.array(Ca)
    Ca_Ca_eq = np.array(Ca_Ca_eq)
    fd = np.array(fd)
...
def plot_erosion_slope_width_over_distance(plotdir_co2, co2, noco2, every=5):
    ...
    cmap = mpl.get_cmap("nipy_spectral")
    n_steps = max(
        [
            int(np.ceil(co2["Ca"].shape[0] / every)),
            int(np.ceil(noco2["Ca"].shape[0] / every)),
        ]
    )
    cyc = mpl.cycler(color=[cmap(k) for k in np.linspace(0, 1, n_steps)])
    mpl.rcParams["axes.prop_cycle"] = cyc

    max_years = max([co2["years"][-1], noco2["years"][-1]])
    cNorm = mpl.colors.Normalize(vmin=0, vmax=max_years)
    ...
    cb = mpl.colorbar(sMap, ax=axs[0, 1], format=cb_formatter)
    cb.set_label("Years")
    ...
    cb = mpl.colorbar(sMap, ax=axs[1, 1], format=cb_formatter)
    cb.set_label("Years")
    ...
    cb = mpl.colorbar(sMap, ax=axs[2, 1], format=cb_formatter)
    cb.set_label("Years")
    axs[2, 1].grid("--")
    axs[2, 1].tick_params(direction="in", labelsize=10)

    axs[3, 0].plot(
        co2["x"].transpose()[::, ::every], abs(co2["Ca_Ca_eq"].transpose()[::, ::every])
    )
    ...
    axs[3, 1].plot(
        noco2["x"].transpose()[::, ::every],
        abs(noco2["Ca_Ca_eq"].transpose()[::, ::every]),
    )
    axs[3, 1].set_xlabel("Distance upstream (m)")
    cb = mpl.colorbar(sMap, ax=axs[3, 1], format=cb_formatter)
    cb.set_label("Years")
    axs[3, 1].grid("--")
    axs[3, 1].tick_params(direction="in", labelsize=10)

    mpl.tight_layout(h_pad=0.0, w_pad=0.0)

    mpl.savefig(plotdir_co2 + "/1-Erosion-slope-width-v-distance.png")
...
def plot_ca_co2_over_time(plotdir_co2, co2, noco2):
    ...
    cNorm = mpl.colors.Normalize(vmin=0, vmax=co2["x"][0, -1])
    sMap = mpl.cm.ScalarMappable(norm=cNorm, cmap=cmap)
    ...
    cb = mpl.colorbar(sMap, ax=axs[0, 1])
    ...
    cb = mpl.colorbar(sMap, ax=axs[1, 1])
    cb.set_label("Distance upstream (m)")
    axs[1, 1].tick_params(labelsize=10)

    mpl.tight_layout(h_pad=0, w_pad=0)
    mpl.savefig(plotdir_co2 + "/1-Ca-CO2-v-time.png")
...
def plot_morphodynamics(plotdir_co2, co2, noco2):
    ...
    cb = mpl.colorbar(sMap, ax=axs[2, 1])
    cb.set_label("Distance upstream (m)")
    ...
    axs[3, 0].plot(
        [co2["years"][0], co2["years"][-1]],
        [-co2["dz_dt_base"], -co2["dz_dt_base"]],
        "--k",
    )
    ...
    cb = mpl.colorbar(sMap, ax=axs[3, 1])
    cb.set_label("Distance upstream (m)")
    axs[3, 1].plot([0, 100000], [0.00125, 0.00125], "k--")
    axs[3, 1].xaxis.set_major_formatter(ticker.FuncFormatter(power_10_label))
    axs[3, 1].plot(
        [noco2["years"][0], noco2["years"][-1]],
        [-noco2["dz_dt_base"], -noco2["dz_dt_base"]],
        "--k",
    )
    axs[3, 1].tick_params(labelsize=10)

    mpl.tight_layout(h_pad=0, w_pad=0)
    mpl.savefig(plotdir_co2 + "/1-Morphodynamics.png")


def final_step_morphology(plotdir_co2, co2, noco2):

    mpl.figure()
    mpl.semilogy(co2["width"][-1], co2["slope"][-1], "ko")
    mpl.semilogy(noco2["width"][-1], noco2["slope"][-1], "bs")
    w_i = co2["width"][-1][0]
    s_i = co2["slope"][-1][0]
    w_f = co2["width"][-1][-1]
    ...
    mpl.plot(w_opt, s_opt, "--k")
    m, b, r, p, err = linregress(np.log10(co2["width"][-1]), np.log10(co2["slope"][-1]))
    mpl.plot(co2["width"][-1], co2["width"][-1] ** m * 10 ** b, ":k")
    ...
    mpl.legend(["With CO2", "Without CO2", "Optimal: " + opt_str, "Fit: " + fit_str])
    mpl.tight_layout()
    mpl.savefig(plotdir_co2 + "/1-Final-Morphology.png")
