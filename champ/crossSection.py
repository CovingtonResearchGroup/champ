...
class CrossSection:
    ...
    def findLR(self, h):
        ...
        whereL = where(condL)
        whereR = where(condR)
        if len(whereL[0]) > 0 and len(whereR[0]) > 0:
            L = whereL[0][0] + 1
            R = whereR[0][0]
        else:
            # If conduit is full, then set L and R to first and last idx
            L = 0
            R = self.n - 1
        return L, R
    ...
    def erode_power_law(
        self, a=1.0, K=1e-5, dt=1.0, T_c=0, no_erode=False, trim=True, resample=True
    ):
